...
class ReactHoverGrid extends React.Component {
  static STRING_OR_ARRAY = [PropTypes.string, PropTypes.array];
  ...
  _serverRenderCheck() {
    if (this.props.server_render_ssr) {
      ...
      invariant(
        ...
        "ReactHoverGrid, server_screen_size must be an array if server_render_ssr, it is a " + typeof this.props.server_screen_size
      );
      ...
      invariant(
        ...
        "ReactHoverGrid, server_grid_size must be an array if server_render_ssr, it is a " + typeof this.props.server_grid_size
      );
  ...
  _captureContainerSizes(window_width, width_container_start) {
  ...
  _tileViewCss() {
    ...
    let tile_image = ` .${TILE_IMAGE_class}{overflow:hidden` + `;height:${hover_grid_row_height}} `;
    return tile_image;
  }

  _defaultTileHoverCss() {
    const hover_class = this.class_id_names["TILE_HOVER_TEXT_PF"];
    let tile_hover_text = ` .${hover_class}{pointer-events:none` + `;opacity:1` + `;position:absolute` + `;height:100%` + `;width:100%} `;
  ...
  _tileContainerCss() {
    ...
    let tile_cont = ` .${tile_cont_class}{margin:${tile_edge}` + `;position:relative` + `;float:left` + `;cursor:pointer` + `;padding:0` + `;overflow:hidden} `;
    return tile_cont;
  }
  ...
  _locationCss() {
    ...
    const tile_parent = ` .${tile_class}{display:table` + `;width:100%` + `;height:${hover_grid_row_height_px} }`;

    const middle = ` .middle${this.html_grid_id}{display:table-cell` + `;vertical-align:middle` + `;text-align:center} `;

    const name_north_west = gradient_locations.NORTH_WEST + this.html_grid_id;
    const north_west =
      ` .${name_north_west}{display:table-cell` + `;vertical-align:top` + `;padding:${ver_text_edge_px} 0 0 ${hor_text_edge_px}` + `;text-align:left} `;

    const name_north = gradient_locations.NORTH + this.html_grid_id;
    const north = ` .${name_north}{display:table-cell` + `;vertical-align:top` + `;padding:${ver_text_edge_px} 0 0` + `;text-align:center} `;

    const name_north_east = gradient_locations.NORTH_EAST + this.html_grid_id;
    const north_east =
      ` .${name_north_east}{display:table-cell` + `;padding:${ver_text_edge_px} ${hor_text_edge_px} 0` + `;vertical-align:top` + `;text-align:right} `;

    const name_east = gradient_locations.EAST + this.html_grid_id;
    const east = ` .${name_east}{display:table-cell` + `;padding:0 ${hor_text_edge_px} 0` + `;vertical-align:middle` + `;text-align:right} `;

    const name_south_east = gradient_locations.SOUTH_EAST + this.html_grid_id;
    const south_east =
      ` .${name_south_east}{display:table-cell` + `;padding:0 ${hor_text_edge_px} ${ver_text_edge_px}` + `;vertical-align:bottom` + `;text-align:right} `;

    const name_south = gradient_locations.SOUTH + this.html_grid_id;
    const south = ` .${name_south}{display:table-cell` + `;padding:0 0 ${ver_text_edge_px}` + `;vertical-align:bottom` + `;text-align:center} `;

    const name_south_west = gradient_locations.SOUTH_WEST + this.html_grid_id;
    const south_west =
      ` .${name_south_west}{display:table-cell` + `;padding:0 0 ${ver_text_edge_px} ${hor_text_edge_px}` + `;vertical-align:bottom` + `;text-align:left} `;

    const name_west = gradient_locations.WEST + this.html_grid_id;
    const west = ` .${name_west}{display:table-cell` + `;padding:0 0 0 ${hor_text_edge_px}` + `;vertical-align:middle` + `;text-align:left} `;
    let location_css = tile_parent + north_west + north + north_east + west + middle + east + south_west + south + south_east;
    return location_css;
  }
  ...
  _renderServerMultiples(google_font_links) {
    ...
    for (let browser_width in this.state.pictures_tiles_rows_server) {
      ...
      all_server_widths.push(
        <div id={id_name} key={browser_width} style={styles_of_tile}>
          {picture_list}
        </div>
      );
    }
    ...
    let multiple_widths = (
      <div style={edge_styles} data-from="ReactHoverGrid._renderServerMultiples">
  ...
  _renderBrowserExact(google_font_links) {
    ...
    return (
      <div style={border_adjust} data-from="ReactHoverGrid._renderBrowserExact">
        <div>
          <style dangerouslySetInnerHTML={{ __html: this.props.inject_css_rules }} />
          <div dangerouslySetInnerHTML={{ __html: google_font_links }} />
          <style dangerouslySetInnerHTML={{ __html: this.injected_css_styles }} />
